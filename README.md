...
## Assessing read and assembly quality after an ONT Nanopore run 
...
zcat output/graphtyper_vcfconcatenate/_no_group_defined__GCF_032697525_1.vcf.gz | tail -n +90 | head -n 2
```
...
<summary> 💡 Learn more here </summary>
  ...
  The main output file `_no_group_defined__GCF_032697525_1.vcf.gz` contains the concatenated variant data from all input VCF files. This file is essential for downstream analyses that require a comprehensive view of variants across multiple samples or regions.
...
zcat output/vcflib_vcffilter/_no_group_defined__GCF_032697525_1.vcffilter.vcf.gz | tail -n +97 | head -n 2
```
...
<summary> 💡 Learn more here </summary>
  ...
  The main output file `_no_group_defined__GCF_032697525_1.vcffilter.vcf.gz` contains the filtered variant data. This file includes only the variants that meet the specified criteria, improving the quality and reliability of the variant calls.
...
### Key output directory: `output/pirate/_no_group_defined__results/`
...
ls -lthr output/pirate/_no_group_defined__results/

# Take a look at one of the output files:
head -n 20 output/pirate/_no_group_defined__results/PIRATE.pangenome_summary.txt
```
